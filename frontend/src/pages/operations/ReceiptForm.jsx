...
const ReceiptForm = () => {
    ...
    const fetchOptions = async () => {
        try {
            const [contactsRes, locationsRes, productsRes] = await Promise.all([
                axios.get('/contacts?type=vendor'),
                axios.get('/locations'),
                axios.get('/products')
            ]);
    ...
    const handleCreateContact = async (name) => {
        try {
            const response = await axios.post('/contacts', { name, type: 'vendor' });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post('/receipts', payload);
    ...
    const handleStatusChange = async (newStatus) => {
        ...
        try {
            if (newStatus === 'ready') {
                await axios.put(`/receipts/${id}/confirm`);
            } else if (newStatus === 'done') {
                await axios.put(`/receipts/${id}/validate`);
            }
            fetchReceipt(); // Refresh data
        } catch (err) {
            setError(`Failed to ${newStatus === 'ready' ? 'confirm' : 'validate'} receipt`);
            console.error(err);
        } finally {
            setSaving(false);
        }
                ...
                <Box>
                    ...
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                        <Box display="flex" alignItems="center" gap={2}>
                            <IconButton onClick={() => navigate('/operations/receipts')} sx={{ bgcolor: 'white', boxShadow: 1 }}>
                                <ArrowBackIcon />
                            </IconButton>
                            <Box>
                                <Typography variant="h5" fontWeight="bold" color="text.primary">
                                    {isEdit ? formData.reference : 'New Receipt'}
                                </Typography>
                                <Typography variant="body2" color="text.secondary">
                                    {isEdit ? `Created on ${new Date().toLocaleDateString()}` : 'Create a new incoming shipment'}
                                </Typography>
                            </Box>
                        </Box>
                    ...
                    <Grid container spacing={3}>
                                ...
                                <Grid container spacing={3}>
                                    <Grid item xs={12} md={6}>
                                        <Autocomplete
                                            ...
                                            disabled={isReadOnly}
                                            renderInput={(params) => <TextField {...params} label="Vendor" required fullWidth />}
                                        />
                                    </Grid>
                                    <Grid item xs={12} md={6}>
                                        <Autocomplete
                        ...
                        {/* Status Card (Optional, maybe for future use or summary) */}
                        <Grid item xs={12} md={4}>
                            <Paper sx={{ p: 3, borderRadius: 3, boxShadow: '0 4px 20px rgba(0,0,0,0.05)', height: '100%', bgcolor: 'primary.main', color: 'white' }}>
                                ...
                                <Typography variant="body1" sx={{ opacity: 0.9, mb: 4 }}>
                                    Current status of this receipt is <strong>{formData.status.toUpperCase()}</strong>.
                                </Typography>
                                {formData.status === 'draft' && (
                                    <Typography variant="body2" sx={{ opacity: 0.8 }}>
                                        Draft receipts are editable. Confirm to reserve stock.
                                    </Typography>
                                )}
                                {formData.status === 'ready' && (
                                    <Typography variant="body2" sx={{ opacity: 0.8 }}>
                                        Receipt is ready. Products are marked as "Free to Use". Validate when goods are physically received.
                                    </Typography>
                                )}
                                {formData.status === 'done' && (
                                    <Typography variant="body2" sx={{ opacity: 0.8 }}>
                                        Receipt is validated. Stock has been added to "On Hand".
                                    </Typography>
                                )}
                            </Paper>
                        </Grid>
